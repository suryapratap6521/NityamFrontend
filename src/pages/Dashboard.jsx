...
const Dashboard = () => {
  ...
  useEffect(() => {
    // Only fetch data if not loading
  ...
  // Early return after useEffect to ensure hooks are called in the same order
  if (profileLoading || authLoading) {
    return <Loader />;
  }
        ...
        {!isSmallScreen && (
          ...
          <div className="w-3/12 bg-white p-4  hidden lg:flex  flex-col items-center">
            <div className="bg-[#fafafa] w-full px-4 py-4 rounded-lg border-2  border-gray-200 max-w-[350px] ">
              ...
              {serviceData && serviceData.length != 0 && serviceData.allUsers.slice(0, 5).map((users, index) => (<>
                <div className="w-full mt-4 flex items-center justify-between gap-2">
                  <div className="w-9 h-9">
                    <img className='bg-gray-400 w-full h-full rounded-full' src={users.image} />
                  </div>
